...
MAIN_TFS=["15m","1h","4h","1d","1w"]
ENTRY_TFS=["3m","5m","15m"]
TF_MS={"3m":180000,"5m":300000,"15m":900000,"1h":3600000,"4h":14400000,"1d":86400000,"1w":604800000}
...
def klines(symbol, interval, limit=120):
    ...
    out=[]
    for x in r.json():
        out.append({"ot":x[0],"o":float(x[1]),"h":float(x[2]),"l":float(x[3]),"c":float(x[4]),"v":float(x[5]),"qv":float(x[7]),"ct":x[6]})
    return out
...
def compact_volume(value):
    """Compact quote-volume display: 550K, 4M, 1B."""
    try:
        v=float(value)
    except Exception:
        return "0"
    ...
    if abs(n) >= 10:
        return f"{n:.1f}{suffix}".rstrip("0").rstrip(".") + suffix if False else f"{n:.1f}".rstrip("0").rstrip(".")+suffix
    return f"{n:.2f}".rstrip("0").rstrip(".")+suffix


def get_24h_quote_volume(symbol):
    """Binance Futures rolling 24h quote volume in USDT for this symbol."""
    try:
        r=S.get(BASE+"/fapi/v1/ticker/24hr",params={"symbol":symbol},timeout=5)
        r.raise_for_status()
        return float(r.json().get("quoteVolume",0) or 0)
    except Exception as e:
        print("24h volume error",symbol,e)
        return 0.0
...
def make_setup(symbol,tf,pname,side,strength,cs):
    ...
    setups[key]={"symbol":symbol,"main_tf":tf,"main_pattern":pname,"side":side,"strength":strength,
                 ...
                 "main_quote_volume":c.get("qv", c.get("v", 0.0)*c.get("c", 0.0)),
                 "expires_at":int(c["ct"])+CONFIRM_WINDOW_SECONDS*1000}
...
def maybe_entry(st, etf, cs, pname, side):
    ...
    group_ded=("ALERT_GROUP",st["symbol"],st["main_tf"],st["side"],st["main_close"])
    with lock:
        if ded in last_entry or group_ded in last_entry: return
    entry=px
    A=atr(cs)
    if side=="BUY":
    ...
    # Reserve this symbol/main-candle/side atomically so parallel confirmation
    # checks cannot emit two Telegram messages for the same trade idea.
    with lock:
        if ded in last_entry or group_ded in last_entry: return
        last_entry[ded]=1
        last_entry[group_ded]=1
...
📊 حجم تداول 24H (USDT): <b>{compact_volume(get_24h_quote_volume(st["symbol"]))}</b>
...
def expected_latest_close_time(now_ms, tf):
    ...
    Binance weekly futures klines are aligned to Monday 00:00 UTC, not to
    Unix-epoch 7-day buckets, so 1w needs its own boundary calculation.
    """
    if tf == "1w":
        # 1970-01-05 00:00:00 UTC was a Monday. This anchor keeps weekly
        # boundaries aligned with Binance: Monday 00:00 UTC open,
        # Sunday 23:59:59.999 UTC close.
        week_ms=TF_MS["1w"]
        monday_anchor_ms=4*86400000
        return ((now_ms-monday_anchor_ms)//week_ms)*week_ms + monday_anchor_ms - 1
    ms=TF_MS[tf]
...
def scan_main_symbol(symbol, tf, target_close, now_ms):
    ...
    try:
        ...
        pats=patterns(use)
        made=0
        # One setup per symbol / main timeframe / side / candle.
        # If several candlestick names describe the same reversal, keep them
        # together and use the strongest score instead of sending duplicates.
        grouped={}
        for mp,side,strength in pats:
            grouped.setdefault(side,[]).append((mp,strength))
        for side,items in grouped.items():
            items=sorted(items,key=lambda x:x[1],reverse=True)
            names=" + ".join(x[0] for x in items)
            strength=max(x[1] for x in items)
            st_key=(symbol,tf,names,side,int(c["ct"]))
            if st_key not in setups:
                make_setup(symbol,tf,names,side,strength,use)
                made+=1
...
def setup_still_valid(st, eclosed, main_close):
    """Final anti-stale / anti-momentum check before Telegram alert.
    The main candlestick idea is cancelled if price invalidated the zone or
    if the post-close candles show a clear impulse in the opposite direction.
    This is deliberately local; it is NOT a global trend filter.
    """
    post=[x for x in eclosed if int(x["ct"])>int(main_close)]
    if not post:
        return True
    A=max(atr(eclosed),1e-12)
    zl,zh=float(st["zone_low"]),float(st["zone_high"])
    invalid=float(st["invalid"])
    if st["side"]=="SELL":
        # Hard invalidation or acceptance above the sell zone.
        if any(x["h"]>=invalid for x in post): return False
        if any(x["c"]>zh+0.15*A for x in post): return False
        last=post[-3:]
        strong=[x for x in last if bull(x) and body(x)>=0.55*rng(x) and x["c"]>=x["l"]+0.70*rng(x)]
        if len(strong)>=2 and last[-1]["c"]-last[0]["o"]>=0.45*A: return False
    else:
        if any(x["l"]<=invalid for x in post): return False
        if any(x["c"]<zl-0.15*A for x in post): return False
        last=post[-3:]
        strong=[x for x in last if bear(x) and body(x)>=0.55*rng(x) and x["c"]<=x["l"]+0.30*rng(x)]
        if len(strong)>=2 and last[0]["o"]-last[-1]["c"]>=0.45*A: return False
    return True

def confirmation_has_rejection(cs, side):
    """Require the confirmation candle to actually reject/break locally,
    not merely have a candlestick label while opposite momentum dominates.
    """
    if len(cs)<2: return False
    p,c=cs[-2],cs[-1]
    if side=="SELL":
        return bear(c) and (c["c"]<p["l"] or (body(c)>=0.45*rng(c) and c["c"]<=c["l"]+0.35*rng(c)))
    return bull(c) and (c["c"]>p["h"] or (body(c)>=0.45*rng(c) and c["c"]>=c["l"]+0.65*rng(c)))
...
def check_setup_confirmation(st, now_ms):
    ...
    for etf in ENTRY_TFS:
        try:
            ...
            ctx=eclosed[:idx+1]
            # Cancel stale/reversed setups before looking at the candle name.
            if not setup_still_valid(st,ctx,main_close):
                return False
            eps=patterns(ctx)
            for ep,eside,eq in eps:
                if eside!=st["side"]: continue
                if not confirmation_has_rejection(ctx,eside): continue
                maybe_entry(st,etf,ctx,ep,eside)
...
def scan_loop():
    ...
    MAIN: 15m / 1h / 4h / 1d / 1w only, and only after the exact candle closes.
    ...
    while True:
        try:
            ...
            if not initialized:
                ...
                # Prioritize 1H and 4H exactly as requested, then 15m, daily and weekly.
                for tf in ["1h","4h","15m","1d","1w"]:
