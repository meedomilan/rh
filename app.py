...
def build_trade_plan(rows: list[list[Any]], direction: str, price: float) -> dict[str, float]:
    source_atr = atr(rows, 14)
    risk = max(source_atr * STOP_ATR_MULT, price * 0.001)
    zone = source_atr * 0.08
    if direction == "BUY":
        return {
...
class Engine:
    ...
    async def start(self):
        ...
        if SEND_STARTUP_MESSAGE:
            await self.send_telegram(
                "✅ <b>Ahmed Delta Early Predictor v2.0 CONFLICT AWARE بدأ العمل</b>\n\n"
                "🎯 1H يُتوقع من 5M\n"
                "🎯 4H يُتوقع من 15M\n"
    ...
    async def analyze_symbol(self, symbol: str) -> list[Signal]:
        k5, k15, k1h, oi5, oi15, premium, depth = await asyncio.gather(
        ...
        for target_tf, rows, normal_bars, fast_bars, oi_rows in (
            ...
            for direction in ("BUY", "SELL"):
                ...
                price = float(rows[-1][4])
                plan = build_trade_plan(rows, direction, price)
...
app = FastAPI(title="Ahmed Delta Early Predictor v2.0 CONFLICT AWARE", lifespan=lifespan)
...
async def health():
    return {
        ...
        "service": "Ahmed Delta Early Predictor v2.0 CONFLICT AWARE",
...
<title>Ahmed Delta Early Predictor v2.0 CONFLICT AWARE</title>
<style>
...
<body><div class="wrap">
<h1>Ahmed Delta Early Predictor v2.0 CONFLICT AWARE</h1>
