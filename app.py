...
# Independent per-coin analysis: BTC/market direction never forces BUY or SELL.
COIN_STATE_MIN = float(os.getenv("COIN_STATE_MIN", "54"))
COIN_OPPOSITE_BLOCK = float(os.getenv("COIN_OPPOSITE_BLOCK", "62"))
COIN_STATE_GAP = float(os.getenv("COIN_STATE_GAP", "8"))
...
def coin_state(rows: list[list[Any]]) -> dict[str, float]:
    """Score the symbol itself before Delta is allowed to choose a side.

    Uses only this coin's candles: structure, EMA stack, rolling VWAP,
    short momentum and directional volume. No BTC/market-direction input.
    """
    closed = rows[:-1] if len(rows) > 1 else rows
    if len(closed) < 55:
    ...
    # Trend/stack of the coin itself.
    if price > e9 > e21 > e50:
        buy += 14; sell -= 14
    ...
    # Local structure: compare recent swing windows, not the whole market.
    prev_hi, recent_hi = max(highs[-12:-6]), max(highs[-6:])
    prev_lo, recent_lo = min(lows[-12:-6]), min(lows[-6:])
    if recent_hi > prev_hi and recent_lo > prev_lo:  # HH + HL
        buy += 12; sell -= 12
    elif recent_hi < prev_hi and recent_lo < prev_lo:  # LH + LL
        sell += 12; buy -= 12

    # Rolling VWAP of this coin.
    typical = [(float(r[2]) + float(r[3]) + float(r[4])) / 3.0 for r in closed[-20:]]
    v20 = vols[-20:]
    vwap = safe_div(sum(px * v for px, v in zip(typical, v20)), sum(v20), price)
    if price > vwap:
        buy += 7; sell -= 7
    elif price < vwap:
        sell += 7; buy -= 7

    # Immediate expansion/momentum; catches leaders such as PORTAL before fading them.
    mom3 = pct_change(closes[-1], closes[-4]) if len(closes) >= 4 else 0.0
    source_atr = max(atr(closed, 14), 1e-12)
    atr_move = (closes[-1] - closes[-4]) / source_atr if len(closes) >= 4 else 0.0
    if mom3 > 0 and atr_move >= 0.6:
        buy += min(12.0, 5.0 + atr_move * 2.0); sell -= min(12.0, 5.0 + atr_move * 2.0)
    elif mom3 < 0 and atr_move <= -0.6:
        pts = min(12.0, 5.0 + abs(atr_move) * 2.0)
        sell += pts; buy -= pts

    # Directional volume on the last closed candle.
    avg_vol = sum(vols[-21:-1]) / 20.0 if len(vols) >= 21 else 0.0
    last = closed[-1]
    if avg_vol > 0 and vols[-1] >= avg_vol * 1.25:
        if float(last[4]) > float(last[1]):
            buy += 6; sell -= 6
        elif float(last[4]) < float(last[1]):
            sell += 6; buy -= 6

    return {"BUY": clamp(buy), "SELL": clamp(sell)}


def coin_direction_allowed(state: dict[str, float], direction: str) -> bool:
    own = state[direction]
    opposite = state["SELL" if direction == "BUY" else "BUY"]
    # Do not fade a clearly dominant move in the coin itself. Neutral/mixed coins remain eligible.
    if opposite >= COIN_OPPOSITE_BLOCK and opposite - own >= COIN_STATE_GAP:
        return False
    return own >= COIN_STATE_MIN or abs(own - opposite) < COIN_STATE_GAP
...
class Engine:
    ...
    async def start(self):
        ...
        if SEND_STARTUP_MESSAGE:
            await self.send_telegram(
                "✅ <b>Ahmed Delta Early Predictor بدأ العمل</b>\n\n"
                "🎯 1H يُتوقع من 5M\n"
                "🎯 4H يُتوقع من 15M\n"
    ...
    async def send_telegram(self, text: str) -> bool:
        ...
        for attempt in range(4):
            try:
                ...
                log.warning("Telegram attempt %s failed: %s", attempt + 1, error)
    ...
    async def analyze_symbol(self, symbol: str) -> list[Signal]:
        ...
        for target_tf, rows, normal_bars, fast_bars, oi_rows in (
            ...
            for direction in ("BUY", "SELL"):
                # Analyze this coin first. Delta can confirm a side, but cannot fight a strong
                # independent trend/expansion in the symbol itself.
                if not coin_direction_allowed(state, direction):
                    continue
                path_type = None
                path = {}
                ...
                # Coin analysis is confirmation, never a BTC/market-direction filter.
                own_state = state[direction]
                opposite_state = state["SELL" if direction == "BUY" else "BUY"]
                if own_state >= 62 and own_state > opposite_state:
                    score = clamp(score + min(8.0, (own_state - 50.0) * 0.25))
                    recipe.append("CoinStructure")
    ...
    def message(self, signal: Signal, stats: dict[str, Any]) -> str:
        ...
        # Keep the previous message content unchanged.
    ...
    async def evaluate_alerts(self):
        ...
        while self.running:
            try:
                ...
                log.exception("Evaluation worker failed")
...
app = FastAPI(title="Ahmed Delta Early Predictor", lifespan=lifespan)
...
async def health():
    return {
        ...
        "service": "Ahmed Delta Early Predictor",
...
<title>Ahmed Delta Early Predictor</title>
<style>
