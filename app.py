...
def maybe_entry(st, etf, cs, pname, side):
    ...
    pad=(zh-zl)*.35
    if not (zl-pad <= px <= zh+pad): return
    ded=(st["symbol"],st["main_tf"],st["main_pattern"],etf,c["ct"])
    ...
    if side=="BUY":
        ...
        emoji="🟢"; title="دخول شراء — نموذج شموع مؤكد"; zone="منطقة الشراء"
    else:
        sl=max(st["invalid"],max(x["h"] for x in cs[-5:])+.10*A)
        risk=sl-entry
        if risk<=0:return
        t1,t2,t3=entry-risk,entry-2*risk,entry-3*risk
        emoji="🔴"; title="دخول بيع — نموذج شموع مؤكد"; zone="منطقة البيع"
...
💰 العملة: <b>{sym}</b>
🕯 النموذج الرئيسي: <b>{html.escape(st["main_pattern"])}</b>
⏰ فريم النموذج: <b>{st["main_tf"].upper()}</b>
📊 قوة النموذج: <b>{st["strength"]}%</b>

⚡ نموذج تأكيد الدخول: <b>{html.escape(pname)}</b>
⏱ فريم الدخول: <b>{etf.upper()}</b>
✅ أغلقت شمعة التأكيد
        ...
        try:
            ...
            for symbol in syms:
                ...
                for tf in MAIN_TFS:
                    ...
                    except Exception as e: print("main",symbol,tf,e)
                # entry confirmation for active setups
                active=[v for v in list(setups.values()) if v["symbol"]==symbol and time.time()-v["created"]<7*86400]
                if active:
                    for etf in ENTRY_TFS:
                        try:
                            closed=klines(symbol,etf,80)[:-1]
                            for p,side,q in patterns(closed):
                                for st in active:
                                    maybe_entry(st,etf,closed,p,side)
                        except Exception as e: print("entry",symbol,etf,e)
            # prune
            with lock:
                for k in list(setups):
                    if time.time()-setups[k]["created"]>=7*86400: setups.pop(k,None)
