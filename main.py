...
async def init_db() -> None:
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        cols = {row[1] for row in await (await db.execute("PRAGMA table_info(opportunities)")).fetchall()}
        if "first_hit" not in cols:
            await db.execute("ALTER TABLE opportunities ADD COLUMN first_hit TEXT")
        if "first_hit_at" not in cols:
            await db.execute("ALTER TABLE opportunities ADD COLUMN first_hit_at TEXT")
        await db.commit()
...
async def save_signal(sig: Signal) -> tuple[int, bool]:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        # بعد تفعيل الدخول نثبت منطقة الدخول والوقف والأهداف حتى تكون الإحصائية عادلة.
        if existing["entered_at"] is not None:
            return int(existing["id"]), False
            ...
            try:
                ...
                async with aiosqlite.connect(DB_PATH) as db:
                    for r in rows:
                        ...
                        # أول حسم للصفقة: TP1 أم SL. لا نغيره لاحقًا.
                        if not r["first_hit"]:
                            if hit1 and hit_stop:
                                # ticker snapshot لا يستطيع معرفة الترتيب داخل نفس الفاصل؛ نسجلها AMBIGUOUS بدل التخمين.
                                updates.update({"first_hit":"AMBIGUOUS","first_hit_at":ts})
                            elif hit1:
                                updates.update({"first_hit":"TP1","first_hit_at":ts})
                            elif hit_stop:
                                updates.update({"first_hit":"SL","first_hit_at":ts})
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        overall = await (await db.execute("""
            SELECT COUNT(*) total,
                   ...
                   SUM(entered_at IS NOT NULL) entered,
                   SUM(first_hit='TP1') tp1_first,
                   SUM(first_hit='SL') sl_first,
                   SUM(first_hit='AMBIGUOUS') ambiguous_first,
                   SUM(tp1_at IS NOT NULL AND (stop_at IS NULL OR tp1_at < stop_at)) tp1_before_sl,
                   SUM(tp2_at IS NOT NULL AND (stop_at IS NULL OR tp2_at < stop_at)) tp2_before_sl,
                   SUM(tp3_at IS NOT NULL AND (stop_at IS NULL OR tp3_at < stop_at)) tp3_before_sl,
                   SUM(stop_at IS NOT NULL AND (tp1_at IS NULL OR stop_at < tp1_at)) sl_before_tp1,
                   SUM(stop_at IS NOT NULL AND tp1_at IS NOT NULL AND stop_at < tp1_at) tp1_after_sl,
                   AVG(mfe_pct) avg_mfe, AVG(mae_pct) avg_mae
            FROM opportunities
        """)).fetchone()
        by_group = await (await db.execute("""
            SELECT direction,current_stage,COUNT(*) cases,
                   ...
                   SUM(tp3_at IS NOT NULL AND (stop_at IS NULL OR tp3_at < stop_at)) tp3_before_sl,
                   AVG(mfe_pct) avg_mfe, AVG(mae_pct) avg_mae
            FROM opportunities GROUP BY direction,current_stage
            ORDER BY direction,current_stage
        """)).fetchall()
        recent = await (await db.execute("""
            SELECT id,symbol,direction,current_stage,entered_at,entered_price,first_hit,first_hit_at,
                   tp1_at,tp2_at,tp3_at,stop_at,outcome,entry_low,entry_high,stop,tp1,tp2,tp3
            FROM opportunities WHERE entered_at IS NOT NULL ORDER BY id DESC LIMIT 30
        """)).fetchall()
    o=dict(overall)
    decided=(o.get('tp1_first') or 0)+(o.get('sl_first') or 0)
    o['win_rate_first']=round(100*(o.get('tp1_first') or 0)/decided,2) if decided else 0.0
    return {"overall":o,"groups":[dict(x) for x in by_group],"recent":[dict(x) for x in recent]}


@app.get("/dashboard", response_class=HTMLResponse)
@app.get("/", response_class=HTMLResponse)
async def dashboard():
    h=await health(); s=await stats(); o=s['overall']
    status="يعمل ✅" if h['ok'] else "يوجد خطأ ⚠️"
    def n(k): return o.get(k) or 0
    rows=''.join(
        f"<tr><td>#{r['symbol']}.P</td><td>{'شراء' if r['direction']=='BUY' else 'بيع'}</td><td>{r['current_stage']}</td>"
        f"<td>{r['first_hit'] or 'قيد المتابعة'}</td><td>{'✅' if r['tp1_at'] else '—'}</td>"
        f"<td>{'✅' if r['tp2_at'] else '—'}</td><td>{'✅' if r['tp3_at'] else '—'}</td>"
        f"<td>{'🛑' if r['stop_at'] else '—'}</td></tr>" for r in s['recent']
    ) or '<tr><td colspan="8">لا توجد صفقات مفعلة بعد</td></tr>'
    groups=''.join(
        f"<tr><td>{'شراء' if g['direction']=='BUY' else 'بيع'}</td><td>{g['current_stage']}</td><td>{g['cases']}</td>"
        f"<td>{g['entered'] or 0}</td><td>{g['tp1_first'] or 0}</td><td>{g['sl_first'] or 0}</td>"
        f"<td>{g['tp2_before_sl'] or 0}</td><td>{g['tp3_before_sl'] or 0}</td></tr>" for g in s['groups']
    )
...
<title>PRO ANALYST Dashboard</title><meta http-equiv="refresh" content="30"><style>
body{{font-family:Arial;background:#090e1b;color:#eef2ff;margin:0;padding:18px}}.wrap{{max-width:1250px;margin:auto}}.grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(155px,1fr));gap:10px}}
.card{{background:#141c30;border:1px solid #293653;border-radius:14px;padding:15px;margin:10px 0}}.k{{color:#9eabd0;font-size:13px}}.v{{font-size:25px;font-weight:800;margin-top:6px}}table{{width:100%;border-collapse:collapse;font-size:14px}}th,td{{padding:10px;border-bottom:1px solid #27324b;text-align:center}}th{{color:#9fb8ff}}.good{{color:#72e69b}}.bad{{color:#ff8181}}a{{color:#8bb8ff}}
</style></head><body><div class="wrap"><h1>Ahmed Early Explosion PRO ANALYST</h1><div class="card">الحالة: <b>{status}</b> · آخر فحص: {h['last_scan'] or 'لم يبدأ'} · تحديث تلقائي كل 30 ثانية</div>
<div class="grid"><div class="card"><div class="k">كل الفرص</div><div class="v">{n('total')}</div></div><div class="card"><div class="k">دخلت منطقة الدخول</div><div class="v">{n('entered')}</div></div><div class="card"><div class="k">TP1 وصل أولاً</div><div class="v good">{n('tp1_first')}</div></div><div class="card"><div class="k">SL وصل أولاً</div><div class="v bad">{n('sl_first')}</div></div><div class="card"><div class="k">نسبة TP1 قبل SL</div><div class="v">{o.get('win_rate_first',0):.2f}%</div></div><div class="card"><div class="k">TP2 قبل SL</div><div class="v">{n('tp2_before_sl')}</div></div><div class="card"><div class="k">TP3 قبل SL</div><div class="v">{n('tp3_before_sl')}</div></div><div class="card"><div class="k">حسم غير واضح بنفس الفحص</div><div class="v">{n('ambiguous_first')}</div></div></div>
<div class="card"><h3>الإحصائية حسب الاتجاه والمرحلة</h3><div style="overflow:auto"><table><tr><th>الاتجاه</th><th>المرحلة</th><th>الحالات</th><th>دخلت</th><th>TP1 أولاً</th><th>SL أولاً</th><th>TP2 قبل SL</th><th>TP3 قبل SL</th></tr>{groups}</table></div></div>
<div class="card"><h3>آخر الصفقات المفعلة</h3><div style="overflow:auto"><table><tr><th>العملة</th><th>الاتجاه</th><th>المرحلة</th><th>أول وصول</th><th>TP1</th><th>TP2</th><th>TP3</th><th>SL</th></tr>{rows}</table></div></div>
<div class="card"><a href="/stats">JSON Stats</a> · <a href="/opportunities">Opportunities</a> · <a href="/health">Health</a><br><small>يتم احتساب النتيجة فقط بعد دخول السعر منطقة الدخول. أول حسم محفوظ ولا يتغير لاحقًا.</small></div></div></body></html>'''
