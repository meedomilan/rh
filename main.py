...
def signal_message(sig: Signal) -> str:
    stage_title={
        "EARLY":"🟡 داخل/قرب منطقة 4H — مراقبة",
        "CONFIRMED":"🔵 تأكيد رفض من منطقة 4H",
        "EXPLOSION":"🔥 دخول مؤكد من منطقة 4H",
    }[sig.stage]
    ...
    action={
        "EARLY":"👀 الحالة: السعر عند المنطقة — ننتظر رفض/تدفق، لا مطاردة",
...
🏦 منطقة {side} 4H: <b>{fmt_price(float(z.get('zone_low',sig.entry_low)))} – {fmt_price(float(z.get('zone_high',sig.entry_high)))}</b>
...
⚡ درجة التدفق: <b>{sig.explosion_score:.1f}%</b>
...
class Engine:
    ...
    async def start(self):
        ...
        if SEND_STARTUP_MESSAGE:
            await send_telegram(
                ...
                "المنطق: منطقة شراء/بيع 4H ثم رفض/تدفق مبكر ومنع مطاردة الحركة\n"
    ...
    async def analyze_symbol(self, symbol: str) -> list[Signal]:
        ...
        for direction in ("BUY","SELL"):
            ...
            dist=float(zone["distance_atr4"]); near_zone=dist<=ZONE_NEAR_ATR4
            in_zone=bool(zone["in_zone"])

            # منع المطاردة: إذا تحرك السعر بعيدًا من المنطقة في اتجاه الصفقة، لا نرسل إشارة متأخرة.
            if direction=="BUY": chase=(c15["price"]-float(zone["zone_high"]))/max(c15["atr"],1e-12)
            else: chase=(float(zone["zone_low"])-c15["price"])/max(c15["atr"],1e-12)
            if chase>MAX_CHASE_ATR15 and not in_zone:
                continue
            if not near_zone:
                continue

            flow_confirm=(s5>=FLOW_CONFIRM_5M and s15>=FLOW_CONFIRM_15M and (ob["imbalance"]>=55 or c5["cvd_strength"]>=58))
            rejection=max(float(r5["score"]),float(r15["score"]))
            strong_reject=rejection>=REJECTION_STRONG
            reject_confirm=rejection>=REJECTION_CONFIRM
            micro_break=c5["is_breakout"] or c15["is_breakout"] or bool(r5["sweep"]) or bool(r15["sweep"])

            # 4H هو السياق؛ لا ننتظر إغلاقه. نرفض فقط التعارض القوي جدًا.
            if s4<42 and float(zone["structure_score"])<42:
                continue

            if strong_reject and flow_confirm and micro_break:
                stage="EXPLOSION"
            elif reject_confirm or (flow_confirm and dist<=0.15):
                stage="CONFIRMED"
            elif (in_zone or dist<=0.12) and max(s5,s15)>=56:
                stage="EARLY"
            else:
                continue

            explosion_score=clamp(0.40*s5+0.30*s15+0.15*s1+0.15*s4)
            entry_score=clamp(0.33*float(zone["zone_strength"])+0.27*rejection+0.22*s5+0.18*ob["imbalance"] - max(0,chase)*18)
            safety_score=clamp(0.30*s4+0.20*float(zone["structure_score"])+0.18*s1+0.15*(100-ob["spoof_risk"])+0.10*o15["oi_score"]+0.07*fund["funding_score"])
            score=clamp((explosion_score+entry_score+safety_score)/3)

            recipe=[f"تحليل 4H: منطقة {('طلب/شراء' if direction=='BUY' else 'عرض/بيع')} محددة قبل الإشارة"]
            if in_zone: recipe.append("السعر داخل منطقة 4H")
            else: recipe.append(f"السعر قريب من المنطقة {dist:.2f} ATR4")
            if bool(r5["sweep"]) or bool(r15["sweep"]): recipe.append("سحب سيولة ثم استرداد/رفض")
            if rejection>=REJECTION_CONFIRM: recipe.append(f"شموع رفض مؤكدة {rejection:.0f}%")
            if flow_confirm: recipe.append("Delta/CVD والتدفق يؤكدان من 5M/15M")
            if ob["absorption"]>=60: recipe.append("امتصاص محتمل داخل المنطقة")
            if o15["oi_change"]>0.25: recipe.append(f"OI يرتفع {o15['oi_change']:+.2f}%")
            if chase<=0.20: recipe.append("Anti-Late: لا يوجد امتداد متأخر")

            plan=zone_trade_plan(direction,c15["price"],zone,max(c15["atr"],1e-12),c15["swing_low"],c15["swing_high"])
            details={
                "zone_4h":zone,"rejection_5m":r5,"rejection_15m":r15,"chase_atr15":chase,
                "orderbook":ob,"oi_5m":o5,"oi_15m":o15,"oi_1h":o1,"funding":fund,
                "features_5m":c5,"features_15m":c15,"features_1h":c1,"features_4h":c4,
            }
            out.append(Signal(
                symbol=symbol,direction=direction,stage=stage,score=score,
        ...
        # لا نرسل BUY وSELL معًا؛ خذ الأعلى جودة فقط.
        if len(out)>1:
            out.sort(key=lambda s:(s.entry_score,s.score),reverse=True)
            return [out[0]]
        return out
