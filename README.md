...
## v2.7 UNLIMITED STATS
- إزالة حد 10000 للحالات المشابهة.
- الإحصائيات التاريخية والتعلم تستخدم جميع الحالات في قاعدة البيانات.
- /stats يجمع كامل قاعدة البيانات بدون حد.
- رفع سقف عرض /alerts و /learning/checkpoints إلى 1,000,000 عند الطلب.
- لا تغيير على منطق الإشارة أو الوقف أو الأهداف.


## v2.8 ADAPTIVE ENTRY + DASHBOARD
- قبل الإرسال يتم تحليل اتجاه العملة على 1H و4H.
- يمنع BUY/SELL إذا الاتجاه العام يعاكس الإشارة بقوة.
- 1H يبحث عن أفضل نقطة دخول بين 3M/5M/15M.
- 4H يبحث عن أفضل نقطة دخول بين 5M/15M/30M/1H.
- فريم الدخول المختار وجودته يظهران داخل Telegram.
- Stop/TP يبقيان محسوبين من ATR فريم الهدف 1H أو 4H، لكن من نقطة الدخول الجديدة.
- إضافة max_beyond_stop_r لمعرفة كم تجاوز السعر الوقف قبل الرجوع.
- الصفحة الرئيسية `/` أصبحت Dashboard عربيًا مفصلًا ويُرتب الإشارات حسب قوة إحصائية تراعي حجم العينة.
- الإحصائيات ما زالت بلا حد وتستخدم كامل قاعدة البيانات.


## v2.9 STAGE + FIRST OUTCOME STATS
- Telegram shows stage name explicitly:
  NORMAL = EARLY = 🟢 إشارة مبكرة
  FAST_TWO = CONFIRMED = 🔵 إشارة مبكرة قوية
  ABNORMAL_ONE = EXPLOSION = 🔥 إشارة مبكرة جدًا
- Dashboard separates BUY/SELL × 1H/4H × stage.
- Shows TP1-first versus STOP-first and declares which arrived first more often.
- Shows TP1/TP2/TP3 and Stop→TP1/TP2/TP3 recovery.
- Reliability ranking accounts for sample size.
- Historical stats remain unlimited.
